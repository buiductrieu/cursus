...
namespace Cursus.UnitTest.Services
{
    ...
    public class Tests
    {
        ...
        public void Setup()
        {
            ...
            // Kh?i t?o Mock cho IUnitOfWork và thi?t l?p tr? v? InstructorInfoRepository mock
            _unitOfWorkMock = new Mock<IUnitOfWork>();
            _unitOfWorkMock.Setup(u => u.InstructorInfoRepository).Returns(_instructorInfoRepositoryMock.Object);

            // Kh?i t?o InstructorService v?i các Mock
        ...
        {
            // Arrange: T?o d? li?u gi? l?p cho vi?c thêm Instructor
            ...
            // Gi? l?p vi?c t?o user thành công b?ng UserManager
            ...
            // Act: G?i hàm InstructorAsync
            var result = await _instructorService.InstructorAsync(registerInstructorDTO);

            // Assert: Ki?m tra k?t qu? tr? v? có thành công hay không
        ...
        {
            // Arrange: T?o DTO v?i các tr??ng r?ng
            var dto = new RegisterInstructorDTO();

            // Act: Ki?m tra tính h?p l? c?a DTO
            var isValid = Isvalid(dto, out var validationResults);

            // Assert: ??m b?o DTO không h?p l? và có l?i
            Assert.IsFalse(isValid);
            Assert.AreEqual(13, validationResults.Count); // Có 13 tr??ng Required trong DTO
        }

        [Test]
        public void RegisterInstructorDTO_InvalidEmailFormat_ReturnsValidationError()
        {
            // Arrange: T?o DTO v?i email không h?p l?
            var dto = new RegisterInstructorDTO
            {
                Email = "invalid-email", // Email không h?p l?
                Password = "Password123!",
        ...
        {
            // Arrange: T?o DTO v?i Password và ConfirmPassword không kh?p
            var dto = new RegisterInstructorDTO
            {
                Email = "test@example.com",
                Password = "Password123!",
                ConfirmPassword = "DifferentPassword!", // Password không kh?p
                FullName = "John Doe",
                Birthday = DateTime.Now,
                Phone = "1234567890",
                Address = "123 Street",
        ...
        {
            // Arrange: T?o DTO v?i s? ?i?n tho?i không h?p l?
            var dto = new RegisterInstructorDTO
            {
                Email = "test@example.com",
                Password = "Password123!",
                ...
                FullName = "John Doe",
                Birthday = DateTime.Now,
                Phone = "InvalidPhone", // S? ?i?n tho?i không h?p l?
                Address = "123 Street",
                TaxNumber = "1234567890",
                CardName = "Visa",
                CardProvider = "Visa Provider",
        ...
        {
            // Arrange: T?o DTO v?i Tax Number không h?p l? (ph?i là 10 ch? s?)
            var dto = new RegisterInstructorDTO
            {
                Email = "test@example.com",
                Password = "Password123!",
                ...
                Phone = "1234567890",
                Address = "123 Street",
                TaxNumber = "1234", // Tax Number không h?p l?
                CardName = "Visa",
                CardProvider = "Visa Provider",
                CardNumber = "1234567812345678",
                SubmitCertificate = "Certificate"
